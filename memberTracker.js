...
class MemberTracker {
    ...
    async removeExcessMembers(thread, maxMembers) {
        try {
            ...
            if (threadConfig.removalState && threadConfig.removalState.remainingMembers && threadConfig.removalState.remainingMembers.length > 0) {
            ...
            // Initialize removal state if it doesn't exist or if remainingMembers is empty
                ...
                for (const [id, threadMember] of threadMembers) {
                    try {
                        ...
                        logWithTimestamp(`Adding member to removal list: ${guildMember.user.username} (${guildMember.id})`, 'DEBUG');
                ...
                logWithTimestamp(`Prepared ${sortedMembers.length} members for batch removal in thread ${thread.name}. Non-guild members: ${categorizedMembers.nonGuildMembers.length}, Regular members: ${categorizedMembers.regularMembers.length}, Skipped: ${skippedCount}`, 'INFO');
